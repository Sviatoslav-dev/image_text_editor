...
class Filter(QWidget):
    ...
    def click_n(self):
        if not np.array_equal(self.img_class.grand_img_copy, self.img_class.img):
            msg = QMessageBox.question(self, "Cancel edits",
                                       "Confirm to discard all the changes?   ",
    ...
    def click_crop(self, rotate=False):
        ...
        def change_slide():
            ...
            self.rb.setGeometry(int(self.img_class.left * self.zoom_factor),
                                ...
                                int((
                                                self.img_class.right - self.img_class.left) * self.zoom_factor),
                                int((
                                                self.img_class.bottom - self.img_class.top) * self.zoom_factor))
...
class Crop(QWidget):
    def __init__(self):
        ...
        self.rotatect.setIcon(
            QIcon(f"{pathlib.Path(__file__).parent.absolute()}\\icon/rotatect90.png"))
...
class ResizableRubberBand(QWidget):
    ...
    def paintEvent(self, event):
        ...
        qp = QPainter(self)
        qp.drawRoundedRect(0, 0, window_size.width(), window_size.height(), self.borderRadius,
                           self.borderRadius)
    ...
    def mouseMoveEvent(self, event):
        if self.draggable and event.buttons() & Qt.LeftButton:
            if self.right <= int(self.img_class.img.shape[1] * self.zoom_factor) and self.bottom <= \
                    int(self.img_class.img.shape[
                            0] * self.zoom_factor) and self.left >= 0 and self.top >= 0:
